...
import { Shirt, TvMinimal, Sofa } from "lucide-react";
import shoppingMan from "../assets/shoppingman.png"
import "../animations.css"

type ItemType = "Clothing" | "Electronics" | "Furniture" | "Material-based-purchases";
...
const fetchTodaysItems = async () => {
  try {
    const response = await fetch ("http://localhost:3000/shopping/todaysshopping", { credentials: 'include' })
...
const handleAddItem = async () => {
  if (!selectedItem || !quantity || !selectedItemType) return;
    ...
    try {
      const response = await fetch('http://localhost:3000/shopping/uploaditem', {
        method: 'POST',
...
const handleDeleteItem = async (itemId: string) => {
  try {
    const response = await fetch(`http://localhost:3000/shopping/deleteitem/${itemId}`, {
      method: 'DELETE',
      credentials: 'include',
    });
  ...
  return (
    <>
      <div className="mr-[10vw] ml-[10vw] mt-[2vh] max-[1200px]:mr-[2vw] max-[1200px]:ml-[2vw] ">
        ...
        <section style={{background:"#FAF0E6"}} className="rounded-lg shadow-lg p-6 mb-6 mt-6">
          ...
          <img src={shoppingMan} alt="mascot riding bus" className="h-96 min-[2000px]:h-[550px] w-auto rounded-tr-lg rounded-tl-[40px] rounded-bl-lg rounded-br-lg"/>
          <h4 className="max-[1000px]:hidden text-gray-700 text-lg min-[2000px]:text-2xl leading-relaxed font-semibold max-w-xl min-[2000px]:max-w-3xl text-center bg-green-200 p-12 min-[2000px]:p-16 rounded-full shadow-sm">
            Track your daily shopping carbon footprint with ease. Whether you're buying clothing, electronics, 
            furniture, or other material goods, we help you understand the environmental impact of your purchases. 
            Simply select your item category below, enter the details of what you bought, and we'll calculate the CO₂ emissions for you. 
            Every mindful purchase makes a difference!
          </h4>
          </div>
          <div className="flex justify-between items-center mb-4">
          <h2 className="text-black font-semibold text-2xl ">Todays Consumed emissions</h2>
          ...
          >
            <div className="flex justify-center"><Shirt color="currentColor"></Shirt></div>
          ...
          <button
            ...
            className="transport-button"
          >
            <div className="flex justify-center"><TvMinimal color="currentColor"></TvMinimal></div>
          ...
          <button
            ...
            className="transport-button"
          >
            <div className="flex justify-center"><Sofa color="currentColor"></Sofa></div>
          ...
          >
            <div className="flex justify-center"><Shirt color="currentColor"></Shirt></div>
            <div className="text-sm font-medium">Add Material-based-purchases</div>
          </button>
          </div>
          {/* catagory section */}
            {selectedItemType && (
            ...
            <div className="flex justify-between items-center mb-4">
              <h3 className="text-xl font-semibold text-gray-800">Add {selectedItemType}</h3>
              <button
            ...
            {/* select item section */}
                ...
                >
                  ...
                  {availableItems.map((item) => (
                    <option key={item.name} value={item.name}>
                      {item.name} ({item.kgCO2ePerKg ? `${item.kgCO2ePerKg} kg CO₂e/kg` : `${item.kgCO2ePerItem} kg CO₂e/item`})
                    </option>
                  ))}
                </select>
              </div>
              ...
              {selectedItem && (() => {
                ...
                return (
                  <div className="mb-6">
                    <label className="block text-gray-700 font-medium mb-2">
                      Step 2: Enter {isWeightBased ? 'Weight (kg)' : 'Quantity'}
                    </label>
                    <input
                      type="number"
                      value={quantity}
          ...
          <h3 className="text-lg font-semibold text-gray-800 mb-3">Today's purchases</h3>
          {todaysPurchases.length === 0 ? (
            <p className="text-gray-500 text-center py-8">No items logged yet. Add your first item above!</p>
          ) : (
            <div className="space-y-2">
              {todaysPurchases.map((item) => {
                const isQuantityBased = item.itemType === "Electronics" || item.itemType === "Furniture";
                ...
                return (
                  <div key={item._id} className="flex justify-between items-center p-3 bg-gray-50 rounded-lg">
                    <div>
                      <span className="font-medium text-gray-800">{item.itemType}</span>
                      <span className="text-gray-600 ml-2">• {item.itemName}</span>
                      <span className="text-gray-500 text-sm ml-2">({displayText})</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <div className="text-green-600 font-semibold">{item.emissions.toFixed(2)} kg CO₂e</div>
                      <button
